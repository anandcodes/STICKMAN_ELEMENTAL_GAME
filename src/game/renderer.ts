import type { GameState, Element, Difficulty, ShopTab } from './types';
...
function drawBackdrop(
  ...
  if (!state.graphicsQuality || state.graphicsQuality === 'high') {
    for (let i = 0; i < 5; i++) {
  ...
  } else if (state.graphicsQuality === 'medium') {
    // Only 2 orbs for medium
    for (let i = 0; i < 2; i++) {
        const cx = (Math.sin(time * 0.6 + i) * 0.5 + 0.5) * W;
        const cy = (Math.cos(time * 0.45 + i) * 0.5 + 0.5) * H;
        const radius = 200;
        const orb = ctx.createRadialGradient(cx, cy, 0, cx, cy, radius);
        orb.addColorStop(0, 'rgba(110, 206, 255, 0.05)');
        orb.addColorStop(1, 'rgba(0,0,0,0)');
        ctx.fillStyle = orb;
        ctx.fillRect(0, 0, W, H);
    }
...
function drawPanel(
  ...
  // Outer Shadow for depth - Skip on low quality
  if (!lowQuality) {
    ctx.shadowColor = 'rgba(0, 0, 0, 0.45)';
    ctx.shadowBlur = state.graphicsQuality === 'medium' ? 15 : 35;
    ctx.shadowOffsetY = state.graphicsQuality === 'medium' ? 6 : 12;
  }
  ...
  // Top Accent with Glow - Skip glow on low quality
  ctx.save();
  if (!lowQuality) {
    ctx.shadowColor = accent;
    ctx.shadowBlur = state.graphicsQuality === 'medium' ? 6 : 12;
  }
...
function drawGemIcon(ctx: CanvasRenderingContext2D, state: GameState, x: number, y: number, size: number, nowMs: number) {
  ...
  if (!lowQuality) {
    ...
    ctx.shadowBlur = (state.graphicsQuality === 'medium' ? 5 : 10) * pulse;
  }
...
export function render(
  ...
  if (state.screen === 'challenges') { drawChallengesScreen(ctx, state, W, H); ctx.restore(); return; }
  if (state.screen === 'survivalDifficulty') { drawSurvivalDifficultyScreen(ctx, state, W, H); ctx.restore(); return; }
  if (state.screen === 'shop') { drawShopScreen(ctx, state, W, H); ctx.restore(); return; }
  if (state.screen === 'levelSelect') { drawLevelSelectScreen(ctx, state, W, H); ctx.restore(); return; }
...
function drawHUD(
  ...
  if (isPortraitMobile) {
    ...
    if (bossCompact && bossCompact.state !== 'dead') {
      ...
      drawPanel(ctx, state, bx - 10, by - 18, barW + 20, 48, 9, '#ff90a0');
  ...
  drawPanel(ctx, state, 10, 10, 286, 132, 12, '#7bd3ff');
  ...
  drawPanel(ctx, state, startX - 12, 10, totalW + 24, 72, 12, ELEMENT_COLORS[state.selectedElement]);
  ...
  drawPanel(ctx, state, W - 214, 10, 204, 106, 12, '#88d8ff');
  ctx.fillStyle = '#f3fbff';
  ...
  if (boss && boss.state !== 'dead') {
    ...
    drawPanel(ctx, state, bx - 12, by - 22, barW + 24, 58, 10, '#ff90a0');
    ctx.fillStyle = 'rgba(255,255,255,0.12)';
  ...
  drawPanel(ctx, state, W / 2 - 300, _H - 36, 600, 28, 9, '#7bcaff');
...
function drawLevelIntro(ctx: CanvasRenderingContext2D, state: GameState, W: number, H: number) {
  ...
  drawPanel(ctx, state, W / 2 - 320, H / 2 - 130, 640, 260, 20, '#78cfff');
...
function drawMenuScreen(ctx: CanvasRenderingContext2D, state: GameState, W: number, H: number) {
  ...
  const actions = [
    ...
    { title: "SURVIVAL", subtitle: "Endless Waves", color: '#ff7688', icon: '⚡', screen: 'survivalDifficulty' },
    { title: "SHOP", subtitle: "Upgrades & More", color: '#53b8ff', icon: '🛒', screen: 'shop' },
    { title: "CHALLENGES", subtitle: "Daily Missions", color: '#ffd36a', icon: '🏆', screen: 'challenges' }
  ];
  ...
  actions.forEach((a, i) => {
    ...
    drawPanel(ctx, state, x, y, cardW, cardH, 16, a.color);
    ctx.restore();
    ...
    const completedChallenges = snap.dailies.filter(d => d.completed && !d.claimed).length;

    if (a.screen === 'challenges' && completedChallenges > 0) {
      const badgeSize = 24;
      ctx.save();
      ctx.fillStyle = '#ff3b3b';
      ctx.shadowColor = '#ff3b3b';
      ctx.shadowBlur = 10;
      roundRect(ctx, x + cardW - 35, y - 10, badgeSize, badgeSize, 12);
      ctx.fill();
      ctx.fillStyle = '#ffffff';
      setUiFont(ctx, state, 12, '900');
      ctx.textAlign = 'center';
      ctx.fillText(completedChallenges.toString(), x + cardW - 23, y + 7);
      ctx.restore();
    }
  });

  // Small Settings Icon in top corner
  const sX = W - 60;
  const sY = 30;
  drawPanel(ctx, state, sX, sY, 40, 40, 10);
  ctx.fill();
  ctx.fillStyle = '#ffffff';
  setUiFont(ctx, state, 18, '400');
  ctx.textAlign = 'center';
  ctx.fillText("⚙️", sX + 20, sY + 28);
  ...
  setUiFont(ctx, state, 12, '700');
  ctx.textAlign = 'center';
  ctx.fillText(`BEST: ${state.highScore}  |  GEMS: ${state.gemsCurrency}  |  LEVEL: ${state.furthestLevel + 1}`, W / 2, barY + 30);
}
...
function drawLevelCompleteScreen(ctx: CanvasRenderingContext2D, state: GameState, W: number, H: number, isMobile = false) {
  ...
  const panelW = 740;
  const panelH = 400;
  drawPanel(ctx, state, W / 2 - panelW / 2, H / 2 - 200, panelW, panelH, 24, '#64f0c0');
  ...
  const drawStat = (x: number, label: string, value: string | number, color = '#ffffff', icon?: 'gem') => {
    ...
    if (icon === 'gem') {
      drawGemIcon(ctx, state, x - 32, cy + 44, 13, nowMs);
      ctx.textAlign = 'left';
...
function drawGameOverScreen(ctx: CanvasRenderingContext2D, state: GameState, W: number, H: number, isMobile = false) {
  ...
  drawPanel(ctx, state, px, py, panelW, panelH, 24, UI_THEME.danger);
  ...
  // Retry Area
  drawPanel(ctx, state, retryX, footerY - 28, btnW, btnH, 16, '#62ffbc');
  ctx.fillStyle = '#ffffff';
  setUiFont(ctx, state, 15, '800');
  ctx.fillText(state.endlessWave !== undefined ? 'RETRY WAVE' : 'RETRY LEVEL', retryX + btnW / 2, footerY + 8);

  // Quit Area
  drawPanel(ctx, state, quitX, footerY - 28, btnW, btnH, 16, '#ff6b81');
  ctx.fillStyle = '#ffffff';
...
function drawVictoryScreen(ctx: CanvasRenderingContext2D, state: GameState, W: number, H: number, isMobile = false) {
  ...
  const panelW = 760;
  const panelH = 400;
  drawPanel(ctx, state, W / 2 - panelW / 2, H / 2 - 200, panelW, panelH, 24, '#8ce8ff');
  ...
  const drawStat = (x: number, label: string, value: string | number, color = '#ffffff', icon?: 'gem') => {
    ...
    if (icon === 'gem') {
      drawGemIcon(ctx, state, x - 25, gridY + 38, 12, nowMs);
      ctx.textAlign = 'left';
...
function drawShopScreen(ctx: CanvasRenderingContext2D, state: GameState, W: number, H: number) {
  ...
  const gemX = W / 2 + 300;
  const gemY = 55;
  drawPanel(ctx, state, gemX - 120, gemY - 25, 200, 50, 12, '#ffd37f');
  drawGemIcon(ctx, state, gemX - 90, gemY + 1, 12, nowMs);
  ctx.fillStyle = '#ffffff';
  setUiFont(ctx, state, 22, '800');
  ctx.textAlign = 'left';
  ctx.fillText(String(state.gemsCurrency), gemX - 65, gemY + 8);
  ...
  const tabs: {id: ShopTab, label: string}[] = [
    ...
    { id: 'currency', label: 'GEMS' },
    { id: 'special', label: 'OFFERS' }
  ];
  const tabW = 160;
  const tabStartX = W / 2 - (tabW * 5) / 2;
  const tabY = 120;

  tabs.forEach((tab, i) => {
    const x = tabStartX + i * tabW;
    const isSelected = state.shopTab === tab.id;
    ...
    ctx.fillStyle = isSelected ? '#53b8ff' : 'rgba(255,255,255,0.05)';
    roundRect(ctx, x + 5, tabY, tabW - 10, 44, 12);
    ctx.fill();
    ctx.restore();
    ...
    ctx.textAlign = 'center';
    ctx.fillText(tab.label, x + tabW / 2, tabY + 28);
  });
  ...
  if (state.shopTab === 'upgrades') {
    ...
    for (let i = 0; i < 6; i++) {
        ...
        const maxed = levels[i] >= 5;
        const canAfford = state.gemsCurrency >= costs[i] || maxed;
        ...
        drawPanel(ctx, state, x, y, cardW, cardH, 18, maxed ? '#62eeb8' : (isSelected ? '#53b8ff' : '#22325a'));
        ctx.restore();
        ...
        if (maxed) {
            ...
            drawGemIcon(ctx, state, x + 20 + ctx.measureText(String(costs[i])).width + 15, y + (isMobileLayout ? 82 : 92), 10, nowMs);
        }
    ...
    const bundles = [
        { name: "Pouch of Gems", amount: 50, price: "$0.99", color: '#53b8ff' },
        { name: "Chest of Gems", amount: 250, price: "$3.99", color: '#ffd36a' },
        { name: "Vault of Gems", amount: 1000, price: "$9.99", color: '#62eeb8' },
        { name: "Free Gems", amount: 10, price: "WATCH AD", color: '#ff7688' }
    ];
    const cardW = 340; const cardH = 100; const gap = 20;
    ...
    bundles.forEach((b, i) => {
        ...
        const x = startX + col * (cardW + gap);
        const y = startY + row * (cardH + gap);
        drawPanel(ctx, state, x, y, cardW, cardH, 16, b.color);
        
        ctx.fillStyle = '#ffffff';
        setUiFont(ctx, state, 18, '800');
        ctx.textAlign = 'left';
        ctx.fillText(b.name, x + 20, y + 40);
        
        ctx.fillStyle = b.color;
        setUiFont(ctx, state, 20, '900');
        ctx.fillText(`+${b.amount}`, x + 20, y + 70);
        drawGemIcon(ctx, state, x + 20 + ctx.measureText(`+${b.amount}`).width + 15, y + 62, 10, nowMs);

        ctx.fillStyle = 'rgba(255,255,255,0.2)';
        roundRect(ctx, x + cardW - 120, y + 55, 100, 32, 8);
        ctx.fill();
        ctx.fillStyle = '#ffffff';
        setUiFont(ctx, state, 11, '800');
        ctx.textAlign = 'center';
        ctx.fillText(b.price, x + cardW - 70, y + 76);
    });
  } else if (state.shopTab === 'skins') {
    const skins = [
        { name: "Origin", icon: '👤', color: '#ffffff' },
        { name: "Inferno", icon: '🔥', color: '#ff4400' },
        { name: "Tidal", icon: '💧', color: '#0088ff' },
        { name: "Grounded", icon: '🌿', color: '#66aa33' },
        { name: "Zephyr", icon: '🌪️', color: '#aabbee' },
        { name: "Void", icon: '🌑', color: '#aa00ff' }
    ];
    const cardW = 160; const cardH = 160; const gap = 20;
    ...
    skins.forEach((s, i) => {
        ...
        const x = startX + col * (cardW + gap);
        const y = startY + row * (cardH + gap);
        drawPanel(ctx, state, x, y, cardW, cardH, 16, s.color);
        
        ctx.fillStyle = '#ffffff';
        setUiFont(ctx, state, 48, '400');
        ctx.textAlign = 'center';
        ctx.fillText(s.icon, x + cardW / 2, y + 80);
        
        setUiFont(ctx, state, 14, '800');
        ctx.fillText(s.name.toUpperCase(), x + cardW / 2, y + 120);
        
        ctx.fillStyle = 'rgba(255,255,255,0.1)';
        roundRect(ctx, x + 20, y + 130, cardW - 40, 20, 6);
        ctx.fill();
        ctx.fillStyle = '#ffffff';
        setUiFont(ctx, state, 10, '700');
        ctx.fillText("OWNED", x + cardW / 2, y + 144);
    });
  } else {
    ctx.fillStyle = 'rgba(255,255,255,0.2)';
    setUiFont(ctx, state, 32, '800');
    ctx.textAlign = 'center';
    ctx.fillText("COMING SOON", W / 2, H / 2 + 100);
  }
...
function drawLights(ctx: CanvasRenderingContext2D, state: GameState, W: number) {
  ...
  const isMedium = state.graphicsQuality === 'medium';
  ...
  const pGrad = ctx.createRadialGradient(px, py, 0, px, py, isMedium ? 80 : 120);
  const pColor = ELEMENT_COLORS[state.selectedElement];
  pGrad.addColorStop(0, pColor + '44');
  pGrad.addColorStop(1, 'transparent');
  ctx.fillStyle = pGrad;
  ctx.beginPath();
  ctx.arc(px, py, isMedium ? 80 : 120, 0, Math.PI * 2);
  ctx.fill();
  ...
  for (const p of state.projectiles) {
    const lx = p.x - cam.x;
    const ly = p.y - cam.y;
    const lGrad = ctx.createRadialGradient(lx, ly, 0, lx, ly, isMedium ? 40 : 60);
    lGrad.addColorStop(0, ELEMENT_COLORS[p.element] + '66');
    lGrad.addColorStop(1, 'transparent');
    ctx.fillStyle = lGrad;
    ctx.beginPath(); ctx.arc(lx, ly, isMedium ? 40 : 60, 0, Math.PI * 2); ctx.fill();
  }
  ...
  for (const obj of state.envObjects) {
    ...
    let lightColor = '';
    let size = isMedium ? 40 : 60;

    if (obj.type === 'gem') { lightColor = '#ffcc0044'; size = isMedium ? 35 : 50; }
    else if (obj.type === 'mana_crystal') { lightColor = '#0088ff44'; size = isMedium ? 35 : 50; }
    else if (obj.type === 'portal') { lightColor = state.portalOpen ? '#aa44ff66' : '#5522aa33'; size = isMedium ? 120 : 180; }
    else if (obj.type === 'fire_pit' || obj.state === 'burning') { lightColor = '#ff440044'; size = isMedium ? 60 : 90; }
...
function drawLevelSelectScreen(ctx: CanvasRenderingContext2D, state: GameState, W: number, H: number) {
  drawBackdrop(ctx, state, W, H, ['#0a1230', '#112755', '#14193d']);

  // Larger, modern main panel
  const panelW = 1080;
  const panelH = 630;
  drawPanel(ctx, state, W / 2 - panelW / 2, 30, panelW, panelH, 24, '#74c8ff');
  ...
  const footerW = 800;
  const footerH = 68;
  drawPanel(ctx, state, W / 2 - footerW / 2, H - 90, footerW, footerH, 12, '#86d2ff');

  ctx.textAlign = 'center';
  ctx.fillStyle = '#ffffff';
  setUiFont(ctx, state, 15, '800');
  const isTouch = ('ontouchstart' in window) || navigator.maxTouchPoints > 0;
  const startText = isTouch ? tr(state, 'level_select_tap_start') : tr(state, 'level_select_click_start');
...
function drawRelicSelectionScreen(ctx: CanvasRenderingContext2D, state: GameState, W: number, H: number, isMobile = false) {
  ...
  state.relicChoices.forEach((relic, i) => {
    ...
    drawPanel(ctx, state, rx, ry, cardW, cardH, 20, relic.rarity === 'legendary' ? '#ffcc00' : (relic.rarity === 'rare' ? '#6ad2ff' : '#ffffff'));
...
function drawChallengesScreen(ctx: CanvasRenderingContext2D, state: GameState, W: number, H: number) {
  drawBackdrop(ctx, state, W, H, ['#0a1428', '#0f2a4a', '#1a3a5a']);
  ...
  setDisplayFont(ctx, state, 42, '800');
  ctx.textAlign = 'center';
  ctx.fillText("DAILY CHALLENGES", W / 2, 80);
  ...
  const mProgress = progression.milestoneProgress / progression.milestoneTarget;
  ctx.fillStyle = '#62eeb8';
  roundRect(ctx, mX, mY, mW * Math.min(1, mProgress), mH, 6);
  ctx.fill();
  ...
  progression.dailies.forEach((d, i) => {
    const y = startY + i * (cardH + gap);
    
    drawPanel(ctx, state, W / 2 - cardW / 2, y, cardW, cardH, 16, d.completed ? '#62eeb8' : '#53b8ff');
    ...
    ctx.fill();
    ctx.fillStyle = d.completed ? '#62eeb8' : '#53b8ff';
    roundRect(ctx, pbX, pbY, pbW * d.progress, pbH, 4);
    ctx.fill();
    
    setUiFont(ctx, state, 12, '600');
    ctx.fillStyle = 'rgba(255,255,255,0.7)';
    ctx.fillText(`Progress: ${Math.floor(d.current)} / ${d.target}`, pbX, pbY + 24);
    ...
    if (d.completed && !d.claimed) {
      const cbW = 80; const cbH = 40;
      const cbX = W / 2 + cardW / 2 - 100;
      const cbY = y + 30;
      ctx.fillStyle = '#62eeb8';
      ...
      setUiFont(ctx, state, 12, '800');
      ctx.textAlign = 'center';
      ctx.fillText("CLAIM", cbX + cbW / 2, cbY + 25);
    } else if (d.claimed) {
      ctx.fillStyle = 'rgba(255,255,255,0.3)';
      setUiFont(ctx, state, 12, '700');
      ctx.textAlign = 'right';
      ctx.fillText("CLAIMED", W / 2 + cardW / 2 - 20, y + 55);
    }
  });
...
function drawSurvivalDifficultyScreen(ctx: CanvasRenderingContext2D, state: GameState, W: number, H: number) {
  drawBackdrop(ctx, state, W, H, ['#1a0a28', '#2a0f4a', '#3a1a5a']);
  
  ctx.fillStyle = '#ffffff';
  setDisplayFont(ctx, state, 42, '800');
  ctx.textAlign = 'center';
  ctx.fillText("WAVE SURVIVAL", W / 2, 100);
  ctx.fillStyle = '#a1b6d1';
  setUiFont(ctx, state, 16, '600');
  ctx.fillText("SELECT YOUR DIFFICULTY", W / 2, 140);

  const diffs: Difficulty[] = ['easy', 'normal', 'hard', 'insane'];
  const labels = ['EASY', 'NORMAL', 'HARD', 'INSANE'];
  const colors = ['#62eeb8', '#ffd36a', '#ff7688', '#aa00ff'];
  const descs = [
    "Slower enemies, more health. 1x Multiplier.",
    "Standard experience. 1.5x Multiplier.",
    "Fast enemies, high damage. 2.5x Multiplier.",
  ...
  diffs.forEach((d, i) => {
    ...
    drawPanel(ctx, state, x, y, cardW, cardH, 16, colors[i]);
    ctx.restore();
